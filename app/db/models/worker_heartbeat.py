...
class WorkerHeartbeat(Base):
    ...
    Each running worker (job_worker, scheduler) upserts a row keyed on
    *worker_id* (``"job_worker:<pid>"``).  The row is updated on every
    ``process_next_job()`` call so the UI can detect workers that have
    gone silent.
    """
